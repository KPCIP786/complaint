...
<?php $result1 = $this->db->query("SELECT ga.applicant_id,ga.applicant_name,ga.applicant_address,ga.applicant_cnic,ga.applicant_mobile,gcd.complaint_detail_id,gcd.complaint_date,gcd.complaint_category_id,gcc.cc_id,gcc.cc_name,gcd.complaint_detail,gcd.complaint_title,gcd.sub_project_id,ps.subproject_id,ps.subproject_name,ps.detail,gcd.source_id,s.source_name,gcd.zone_id,gzt.zone_name,gcd.uc_id,gut.uc_name,gcd.nc_id,gnt.nc_name,cf.complaint_id,cf.file FROM gre_applicant AS ga,grm_complaint_category As gcc,gre_complaint_detail As gcd,grm_source_tbl As s,ppms_subproject As ps,tehsil As th,grc_zone_tbl as gzt,grm_uc_tbl as gut,grm_nc_tbl as gnt,complaint_file as cf where ga.applicant_id=gcd.applicant_id AND gcc.cc_id=gcd.complaint_category_id AND s.source_id=gcd.source_id AND ps.subproject_id=gcd.sub_project_id AND gcd.zone_id=gzt.zone_id AND gcd.uc_id=gut.uc_id AND gcd.nc_id=gnt.nc_id AND cf.complaint_id=gcd.complaint_detail_id AND gcd.complaint_detail_id=$aid")->row(); ?>
