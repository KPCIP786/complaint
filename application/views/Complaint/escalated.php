...
$tierid=$this->session->userdata('tierid');
 ?>
<?php 
    $result3 = $this->db->query("SELECT * FROM gre_complaint_detail as gc,grm_tier_tbl as gt where gc.tier_id=1 AND gt.tier_id=1")->result();
    $c=0;
    foreach($result3 as $res){
      $Date2 = date('Y-m-d', strtotime($res->complaint_date . " + $res->tier_days day"));
      if($Date2 < date('Y-m-d')){
       $update = $this->db->query("update gre_complaint_detail set status_id=4 where tier_id=1 AND complaint_date='$res->complaint_date' AND status_id=1");
  }
}
$c++; ?>
<?php 
    $result3 = $this->db->query("SELECT * FROM gre_complaint_detail as gc,grm_tier_tbl as gt where gc.tier_id=2 AND gt.tier_id=2")->result();
    $co=0;
    foreach($result3 as $res){
      $Date2 = date('Y-m-d', strtotime($res->complaint_date . " + $res->tier_days day"));
      if($Date2 < date('Y-m-d')){
       $update = $this->db->query("update gre_complaint_detail set status_id=4 where tier_id=2 AND complaint_date='$res->complaint_date' AND status_id=1");
  }
}
$co++; ?>
<?php 
    $result3 = $this->db->query("SELECT * FROM gre_complaint_detail as gc,grm_tier_tbl as gt where gc.tier_id=3 AND gt.tier_id=3")->result();
    $cou=0;
    foreach($result3 as $res){
      $Date2 = date('Y-m-d', strtotime($res->complaint_date . " + $res->tier_days day"));
      if($Date2 < date('Y-m-d')){
       $update = $this->db->query("update gre_complaint_detail set status_id=4 where tier_id=3 AND complaint_date='$res->complaint_date' AND status_id=1");
  }
}
$cou++; ?>
			...
			<span class="card-label fw-bold text-gray-800">Escalated Complaints</span>
			<!-- <span class="text-gray-400 mt-1 fw-semibold fs-6">Updated 37 minutes ago</span> -->
			   ...
			   if ($group==1){
          $h=1; 
                  $result1 = $this->db->query("SELECT * FROM gre_applicant AS ga,user as u,city as c,grm_complaint_category As gcc,gre_complaint_detail As gcd,grm_source_tbl As s,ppms_subproject As ps,grm_tier_tbl As t,grc_status_tbl As gs,grc_zone_tbl as gzt,grm_uc_tbl as gut,grm_nc_tbl as gnt where ga.applicant_id=gcd.applicant_id AND gcc.cc_id=gcd.complaint_category_id AND s.source_id=gcd.source_id AND ps.subproject_id=gcd.sub_project_id AND t.tier_id=gcd.tier_id AND gs.status_id=gcd.status_id AND gcd.status_id=4 AND gcd.zone_id=gzt.zone_id AND gcd.uc_id=gut.uc_id AND gcd.nc_id=gnt.nc_id AND gcd.user_id=u.user_id AND u.city_id=c.city_id order BY gcd.complaint_detail_id DESC")->result();
...
$firstLetters=mb_strtoupper(implode('',$matches[0]));
 ...
 $h++;
          ...
          $h=1; 
                  $result1 = $this->db->query("SELECT * FROM gre_applicant AS ga,user AS u,city as c,grm_complaint_category As gcc,gre_complaint_detail As gcd,grm_source_tbl As s,ppms_subproject As ps,grm_tier_tbl As t,grc_status_tbl As gs,grc_zone_tbl as gzt,grm_uc_tbl as gut,grm_nc_tbl as gnt where ga.applicant_id=gcd.applicant_id AND gcc.cc_id=gcd.complaint_category_id AND s.source_id=gcd.source_id AND ps.subproject_id=gcd.sub_project_id AND t.tier_id=gcd.tier_id AND gs.status_id=gcd.status_id AND gcd.status_id=4 AND gcd.zone_id=gzt.zone_id AND gcd.uc_id=gut.uc_id AND gcd.nc_id=gnt.nc_id AND gcd.user_id=u.user_id AND u.city_id=$city AND u.city_id=c.city_id order BY gcd.complaint_detail_id DESC")->result();
