 ...
 ?>
          ...
          <?php 
			...
			<!-- <span class="text-gray-400 mt-1 fw-semibold fs-6">Updated 37 minutes ago</span> -->
			   ...
			   if ($group == 1){
			   	$h=1; 
                  $result1 = $this->db->query("SELECT * FROM gre_applicant AS ga,user as un,grm_complaint_category As gcc,gre_complaint_detail As gcd,grm_source_tbl As s,ppms_subproject As ps,grm_tier_tbl As t,grc_status_tbl As gs,grc_zone_tbl as gzt,grm_uc_tbl as gut,grm_nc_tbl as gnt,city as c where ga.applicant_id=gcd.applicant_id AND gcc.cc_id=gcd.complaint_category_id AND s.source_id=gcd.source_id AND ps.subproject_id=gcd.sub_project_id AND t.tier_id=gcd.tier_id AND gcd.tier_id=1 AND gs.status_id=gcd.status_id AND gcd.zone_id=gzt.zone_id AND gcd.uc_id=gut.uc_id AND gcd.nc_id=gnt.nc_id  AND gcd.user_id=un.user_id AND un.city_id=c.city_id order BY gcd.complaint_detail_id DESC")->result();
...
$firstLetters=mb_strtoupper(implode('',$matches[0]));
 ...
 $h++;
                  ...
                  $result1 = $this->db->query("SELECT * FROM gre_applicant AS ga,user AS un,grm_complaint_category AS gcc,gre_complaint_detail AS gcd,grm_source_tbl AS s,ppms_subproject AS ps,grm_tier_tbl AS t,grc_status_tbl AS gs,grc_zone_tbl AS gzt,grm_uc_tbl AS gut,grm_nc_tbl AS gnt,city as c WHERE ga.applicant_id=gcd.applicant_id AND gcc.cc_id=gcd.complaint_category_id AND s.source_id=gcd.source_id AND ps.subproject_id=gcd.sub_project_id AND gcd.tier_id=t.tier_id AND 
                  	  gcd.tier_id=1 AND gs.status_id=gcd.status_id AND gcd.zone_id=gzt.zone_id AND gcd.uc_id=gut.uc_id AND gcd.nc_id=gnt.nc_id AND gcd.user_id=un.user_id AND un.city_id=$city AND un.city_id=c.city_id order BY gcd.complaint_detail_id DESC")->result();
