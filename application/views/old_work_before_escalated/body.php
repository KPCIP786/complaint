...
<body id="kt_app_body" data-kt-app-header-fixed-mobile="true" data-kt-app-sidebar-enabled="true"
 ...
 <!--begin::Main-->
	...
	<table class="table table-bordered align-middle table-stripped" id="kt_inbox_listing" id="kt_customers_table"><!--begin::Table head-->
															<thead>
															<tr>
                                                            <th>Serial No </th>
 ...
 $h++;
               ...
               elseif($group==4) {
			   	$h=1; 
                  $result1 = $this->db->query("SELECT * FROM gre_applicant AS ga,user AS u,grm_complaint_category As gcc,gre_complaint_detail As gcd,grm_source_tbl As s,ppms_subproject As ps,grm_tier_tbl As t,grc_status_tbl As gs,grc_zone_tbl as gzt,grm_uc_tbl as gut,grm_nc_tbl as gnt,city as c where ga.applicant_id=gcd.applicant_id AND gcc.cc_id=gcd.complaint_category_id AND s.source_id=gcd.source_id AND ps.subproject_id=gcd.sub_project_id  AND t.tier_id=gcd.tier_id AND gcd.tier_id=$tierid AND gs.status_id=gcd.status_id AND gcd.zone_id=gzt.zone_id AND gcd.uc_id=gut.uc_id AND gcd.nc_id=gnt.nc_id AND gcd.user_id=u.user_id AND u.city_id=$city AND u.city_id=c.city_id")->result();
 ...
 $h++;
			  	...
			  	$h=1; 
                  $result1 = $this->db->query("SELECT * FROM gre_applicant AS ga,user AS u,grm_complaint_category As gcc,gre_complaint_detail As gcd,grm_source_tbl As s,ppms_subproject As ps,grm_tier_tbl As t,grc_status_tbl As gs,grc_zone_tbl as gzt,grm_uc_tbl as gut,grm_nc_tbl as gnt,city as c where ga.applicant_id=gcd.applicant_id AND gcc.cc_id=gcd.complaint_category_id AND s.source_id=gcd.source_id AND ps.subproject_id=gcd.sub_project_id AND t.tier_id=gcd.tier_id AND (gcd.tier_id=1 or gcd.tier_id=2 or gcd.tier_id=3) AND  gs.status_id=gcd.status_id AND gcd.zone_id=gzt.zone_id AND gcd.uc_id=gut.uc_id AND gcd.nc_id=gnt.nc_id AND gcd.user_id=u.user_id AND u.city_id=$city AND u.city_id=c.city_id")->result();
 ...
 $h++;
  ...
  ?>
   <!-- modal -->
	...
	<table class="table table-bordered align-middle table-stripped" id="kt_inbox_listing" id="kt_customers_table">					
